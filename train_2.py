...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        # self.fc1 = nn.Linear(256 * 3 * 3, 512)  # Adjusted input size based on conv layers
...
class MyMNISTTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        hot_pos_tensor = torch.full_like(img_tensor, -1.0)
        # Set the value at the index to 1
        hot_pos_tensor.view(-1)[index] = num

        # Apply the hot position tensor as a channel
        img_tensor = torch.cat((img_tensor, hot_pos_tensor), dim=0)

        # # Create one-hot encoding for digit class
        # one_hot_label = torch.zeros(10)
        # one_hot_label[num] = 1.0

        # # Broadcast to spatial dimensions - 10 additional channels
        # one_hot_label = one_hot_label.unsqueeze(1).unsqueeze(2).expand(-1, 28, 28)

        # # Concatenate the one-hot label with the image tensor
        # img_tensor = torch.cat((img_tensor, one_hot_label), dim=0)
...
if __name__ == "__main__":
    ...
    model.eval()
    with torch.no_grad():
        # Create a empty tensor to be concatenated (n * 2 * 28 * 28), where n is the batch size
        batched_input = torch.empty((0, 2, 28, 28))

        for i in range(10):
            # Create an 28*28 tensor with -1.0 values
            masked_image = torch.full((1, 28, 28), -1.0)

            # Create a hot position tensor with 1.0 at 0th position
            hot_pos_tensor = torch.full((1, 28, 28), -1.0)
            hot_pos_tensor[0, 0, 0] = i

            # Concatenate the hot position tensor with the masked image
            input_tensor = torch.cat((masked_image, hot_pos_tensor), dim=0)

            # # Create number channel with 0.0 values
            # number_channel = torch.zeros((10, 28, 28))

            # # Set the entire ith channel to 1.0
            # number_channel[i, :, :] = 1.0

            # # Concatenate the number channel with the input tensor
            # input_tensor = torch.cat((input_tensor, number_channel), dim=0)

            # Add the input tensor to the batch
            batched_input = torch.cat((batched_input, input_tensor.unsqueeze(0)), dim=0)

        batched_input = batched_input.cuda()  # Move to GPU if available
        ...
        for i in range(0, 28*28):
            ...
            old = batched_input[:, 1].view(10, -1)[:, i].detach().clone()
            batched_input[:, 1].view(10, -1)[:, i] = -1.0
            try:
                batched_input[:, 1].view(10, -1)[:, i+1] = old
            except IndexError:
                pass
