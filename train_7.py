...
class MyMNISTTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        pixel_index = idx // self.data_len
        img_index = idx % self.data_len
        ...
        pos_encoding_tensor = torch.full_like(img_tensor, pixel_index / 784.0)
...
if __name__ == "__main__":
    ...
    # internal_dataset_size = len(dataset.dataset)
    # for i in range(5):
    #     offset = i * internal_dataset_size
    #     for j in range(offset, offset + 5):
    #         data, target = dataset[j]
    #         print(f"Data shape: {data.shape}, Target: {target}, Index: {j}")
